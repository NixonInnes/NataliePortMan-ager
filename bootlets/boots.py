...
class Boot:
    ...
    def build(self):
        return html.Div(*self.args,
                        **self.get_kwargs())

    def draw(self) -> str:
        return html.try_draw(self.build())
...
class AlertDismissButton(Boot):
    ...
    def build(self):
        return html.Button(html.Span('&times;', aria_hidden='true'),
                        **self.get_kwargs())
...
class ListGroup(Boot):
    ...
    def build(self):
        return html.Ul(*[self.Li(arg) for arg in self.args],
                    **self.get_kwargs())
...
class QuickForm(Boot):
    ...
    def build(self):
        form = self.args[0]
        return html.Form(html.Container(*[
            FormField(field,
                      _form_type=self.get('_form_type'),
                      _columns=self.get('_columns'),
                      _button_map=self.get('_button_map'))
...
class ModalButton(Boot):
    ...
    def build(self):
        return html.Button(type='button',
                           ...
                           data_toggle='modal',
                           data_target=f'#Modal_{self.get("_id")}')(self.get("_text"))
