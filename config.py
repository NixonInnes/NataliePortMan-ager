...
class Config(object):
    ...
    APP_NAME = os.getenv("APP_NAME", "Trade Info")
...
class DevConfig(Config):
    ...
    SQLALCHEMY_DATABASE_URI = os.getenv(
        "DEV_DATABASE_URI", "sqlite:///" + os.path.join(basedir, "dev-db.sqlite")
    )
...
class TestConfig(Config):
    TESTING = True
    SQLALCHEMY_DATABASE_URI = os.getenv(
        "TEST_DATABASE_URI", "sqlite:///" + os.path.join(basedir, "test-db.sqlite")
    )
